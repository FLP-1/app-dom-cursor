  ...
  try {
    ...
    const empregador = await prisma.user.upsert({
      ...
      create: {
        ...
        userType: UserType.EMPREGADOR,
      },
    });

    // Criar usuário comum
    await prisma.user.upsert({
      where: { email: 'comum@dom.com' },
      update: {},
      create: {
        email: 'comum@dom.com',
        name: 'Usuario Comum',
        cpf: '86862836063',
        phone: '11977777777',
        password: await bcrypt.hash('comum123', 10),
    ...
    await prisma.user.upsert({
      where: { email: 'parceiro@dom.com' },
      update: {},
      create: {
        email: 'parceiro@dom.com',
        name: 'Usuario Parceiro',
        cpf: '83244264021',
        phone: '11966666666',
        password: await bcrypt.hash('parceiro123', 10),
        userType: UserType.PARCEIRO,
      },
    });

    // Criar usuário inativo (apenas para exemplo, sem campo de status no schema)
     await prisma.user.upsert({
      where: { email: 'inativo@dom.com' },
      update: {},
      create: {
        email: 'inativo@dom.com',
        name: 'Usuario Inativo',
        cpf: '73133695006',
        phone: '11955555555',
        password: await bcrypt.hash('inativo123', 10),
        userType: UserType.COMUM,
      },
    });
