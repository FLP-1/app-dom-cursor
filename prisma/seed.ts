...
async function main() {
  ...
  const planoBasico = await prisma.plano.upsert({
    ...
    create: {
      id: '1',
      nome: 'Plano Básico',
      mensagem: 'Ideal para pequenos empregadores',
      beneficios: ['Gestão de documentos', 'Controle de ponto', 'Relatórios básicos'],
      valorMensal: 29.90,
      valorAnual: 299.90,
      vigenteDe: new Date(),
      versao: 1,
  ...
  const planoPremium = await prisma.plano.upsert({
    ...
    create: {
      id: '2',
      nome: 'Plano Premium',
      mensagem: 'Recursos avançados para grandes empregadores',
      beneficios: [
        'Todas as funcionalidades do Plano Básico',
        'Gestão de múltiplos empregados',
