import { renderHook, act } from '@testing-library/react-hooks';
import { usePonto } from '@/hooks/usePonto';
import { useNotification } from '@/hooks/useNotification';
import { useConfiguracaoPonto } from '@/hooks/useConfiguracaoPonto';

// Mock dos hooks
jest.mock('@/hooks/useNotification');
jest.mock('@/hooks/useConfiguracaoPonto');

// Mock do fetch
global.fetch = jest.fn();

describe('usePonto', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    
    // Mock do useNotification
    (useNotification as jest.Mock).mockReturnValue({
      error: jest.fn(),
      success: jest.fn()
    });

    // Mock do useConfiguracaoPonto
    (useConfiguracaoPonto as jest.Mock).mockReturnValue({
      configuracao: {
        ativo: true,
        horaInicio: new Date(2024, 0, 1, 8, 0),
        horaFim: new Date(2024, 0, 1, 18, 0),
        latitude: -23.5505,
        longitude: -46.6333,
        raioMetros: 100
      },
      validarHorarioPermitido: jest.fn().mockReturnValue(true),
      validarLocalizacao: jest.fn().mockReturnValue(true)
    });

    // Mock do fetch para /api/time
    (global.fetch as jest.Mock).mockImplementation((url) => {
      if (url === '/api/time') {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ timestamp: Date.now() })
        });
      }
      if (url === 'https://api.ipify.org?format=json') {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ ip: '127.0.0.1' })
        });
      }
      if (url === '/api/ponto/registrar') {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({
            id: '1',
            dataHora: new Date(),
            tipo: 'ENTRADA',
            latitude: -23.5505,
            longitude: -46.6333,
            timezone: 'America/Sao_Paulo',
            dispositivo: 'test',
            ip: '127.0.0.1',
            validado: true
          })
        });
      }
      return Promise.reject(new Error('Not found'));
    });

    // Mock do navigator.geolocation
    Object.defineProperty(global.navigator, 'geolocation', {
      value: {
        getCurrentPosition: jest.fn().mockImplementation((success) => {
          success({
            coords: {
              latitude: -23.5505,
              longitude: -46.6333
            }
          });
        })
      }
    });
  });

  it('deve registrar ponto com sucesso', async () => {
    const { result } = renderHook(() => usePonto());

    await act(async () => {
      await result.current.registrarPonto('ENTRADA');
    });

    expect(result.current.loading).toBe(false);
    expect(result.current.ultimoRegistro).toBeTruthy();
    expect(result.current.ultimoRegistro?.tipo).toBe('ENTRADA');
  });

  it('deve validar horário permitido', async () => {
    const { result } = renderHook(() => usePonto());

    // Mock de horário não permitido
    (useConfiguracaoPonto as jest.Mock).mockReturnValue({
      configuracao: {
        ativo: true,
        horaInicio: new Date(2024, 0, 1, 8, 0),
        horaFim: new Date(2024, 0, 1, 18, 0)
      },
      validarHorarioPermitido: jest.fn().mockReturnValue(false),
      validarLocalizacao: jest.fn().mockReturnValue(true)
    });

    await act(async () => {
      await result.current.registrarPonto('ENTRADA');
    });

    expect(result.current.loading).toBe(false);
    expect(result.current.ultimoRegistro).toBeNull();
  });

  it('deve validar localização', async () => {
    const { result } = renderHook(() => usePonto());

    // Mock de localização fora do raio permitido
    (useConfiguracaoPonto as jest.Mock).mockReturnValue({
      configuracao: {
        ativo: true,
        latitude: -23.5505,
        longitude: -46.6333,
        raioMetros: 100
      },
      validarHorarioPermitido: jest.fn().mockReturnValue(true),
      validarLocalizacao: jest.fn().mockReturnValue(false)
    });

    await act(async () => {
      await result.current.registrarPonto('ENTRADA');
    });

    expect(result.current.loading).toBe(false);
    expect(result.current.ultimoRegistro).toBeNull();
  });

  it('deve validar data/hora do dispositivo', async () => {
    const { result } = renderHook(() => usePonto());

    // Mock de data/hora desatualizada
    (global.fetch as jest.Mock).mockImplementation((url) => {
      if (url === '/api/time') {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ timestamp: Date.now() + 1000 * 60 * 10 }) // 10 minutos de diferença
        });
      }
      return Promise.reject(new Error('Not found'));
    });

    await act(async () => {
      await result.current.registrarPonto('ENTRADA');
    });

    expect(result.current.loading).toBe(false);
    expect(result.current.ultimoRegistro).toBeNull();
  });
}); 