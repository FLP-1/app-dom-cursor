/**
 * Arquivo: empregado.ts
 * Caminho: src/types/empregado.ts
...
export interface EmpregadoResponse {
  data: Empregado;
  message: string;
}

export interface EmpregadoListResponse {
  data: Empregado[];
  total: number;
  page: number;
  limit: number;
...
export interface EmpregadoError {
  message: string;
  code: string;
  field?: string;
} 