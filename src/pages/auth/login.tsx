...
import { Card, CardContent, Button, Typography, Box, IconButton, InputAdornment, Avatar, Chip, CircularProgress } from '@mui/material';
      ...
      <Card sx={{ maxWidth: 400, width: '100%', borderRadius: 3, boxShadow: '0 20px 40px rgba(0,0,0,0.1)', backdropFilter: 'blur(10px)', background: 'rgba(255,255,255,0.95)' }}>
        <CardContent sx={{ p: 4 }}>
          ...
          <Box textAlign="center" mb={3}>
            ...
            <Typography variant="body2" color="text.secondary">Sistema de Gestão Doméstica</Typography>
          </Box>

          <form onSubmit={onSubmit}>
            {/* Seletor de Perfil */}
            <Box mb={3}>
              <Typography variant="subtitle2" mb={1}>Escolha seu perfil:</Typography>
              <Controller
                name="profile"
                control={control}
                render={({ field }) => (
            ...
            <Box>
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <TextField {...field} fullWidth label="Email" variant="outlined" sx={{ mb: 2 }} error={!!errors.email} helperText={errors.email?.message} InputProps={{ startAdornment: (<InputAdornment position="start"><Email color="primary" /></InputAdornment>), sx: { borderRadius: 2 } }} />
                )}
              />
